...
for(ctr in 1:nrow(nps_rec_visits_np_2018_plotly)){
  ...
  id[ctr] <-nps_rec_visits_np_2018_plotly$np_id[ctr]
  l[ctr] <- nps_rec_visits_np_2018_plotly$percent_visit[ctr] + adjust
...
plot.df_p1 <- data.frame(id, x, y, xend, yend, x_len, y_len, segment_len, stringsAsFactors = FALSE)

plot.df_p1 <- plot.df_p1 %>% 
  left_join(y = nps_rec_visits_np_2018_plotly, by = c("id" = "np_id")) %>% 
  arrange(id)
...
p1 <- plot_ly(plot.df_p1,
              type="scatter",
              mode = 'lines',
              name = ~nps_region_names) %>% 
  add_segments(x = ~x, xend = ~xend,
               y = ~y, yend = ~yend,
               color = ~color_map[nps_region],
               line = list(width = 4),
               hoverinfo = 'text',
               text = ~paste(ParkName,
...
for(ctr in 1:nrow(nps_rec_visits_np_2018_plotly)){
  ...
  p2$x$layout$annotations[[ctr]] <- list(x = xend[ctr], y = yend[ctr], showarrow = FALSE,
                                        text = paste0(nps_rec_visits_np_2018_plotly$ParkName[ctr]),
                                        textangle = textangle,
                                        xanchor = xanchor,
                                        yanchor = yanchor,
